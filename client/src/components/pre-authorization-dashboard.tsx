...
export function PreAuthorizationDashboard() {
  ...
  const [selectedRequestForModal, setSelectedRequestForModal] =
    useState<PreAuthRequest | null>(null);
  ...
  // Fetch data
  const { data: preAuthRequests = [], isLoading: loadingRequests } = useQuery<
    PreAuthRequest[]
  >({
  ...
  const { data: procedureRequirements = [] } = useQuery<
    ProcedureAuthRequirement[]
  >({
  ...
  const getMatchingCriteria = (procedureCode: string, payerName: string) => {
    const criteria = insurerCriteria.find(
      (c) => c.procedureCode === procedureCode && c.payerName === payerName,
    );
  ...
  const filteredRequests = preAuthRequests.filter((req) => {
    ...
    // Payer filter
    const matchesPayer =
      !filterPayer || filterPayer === "all" || req.payer === filterPayer;

    // Status filter
    const matchesStatus =
      !filterStatus || filterStatus === "all" || req.status === filterStatus;

    // Days until procedure filter
    const matchesDaysUntil =
      !filterDaysUntil ||
      filterDaysUntil === "all" ||
      (() => {
        const today = new Date();
  ...
  const approvedRequests = preAuthRequests.filter(
    (r) => r.status === "approved",
  ).length;
  const pendingRequests = preAuthRequests.filter(
    (r) => r.status === "pending" || r.status === "requires_review",
  ).length;
  ...
  if (loadingRequests) {
    return (
      <div className="flex items-center justify-center h-64">
        Loading pre-authorization data...
      </div>
    );
  }
    ...
    <div className="flex-1 p-6 space-y-6 bg-white">
        ...
        <TabsContent value="requests" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {Array.from(
                        new Set(preAuthRequests.map((r) => r.procedureCode)),
                      ).map((code) => {
                        const request = preAuthRequests.find(
                          (r) => r.procedureCode === code,
                        );
                        return (
                ...
                <div>
                  ...
                  <Select value={filterPayer} onValueChange={setFilterPayer}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(
                        new Set(preAuthRequests.map((r) => r.payer)),
                      ).map((payer) => (
                ...
                <div>
                  <Label className="text-sm">
                    Filter by Days Until Procedure
                  </Label>
                  <Select
                    value={filterDaysUntil}
                    onValueChange={setFilterDaysUntil}
                  >
              ...
              {/* Filter Summary */}
              {((filterProcedure && filterProcedure !== "all") ||
                (filterPayer && filterPayer !== "all") ||
                (filterStatus && filterStatus !== "all") ||
                (filterDaysUntil && filterDaysUntil !== "all")) && (
                        ...
                        >
                          {isBulkAutoPopulating ? (
                            <>
                              ...
                              <span>
                                Auto-populating... ({bulkProgress.current}/
                                {bulkProgress.total})
                              </span>
                            </>
                          ) : (
                            <>
                              ...
                              <span>
                                Bulk Auto-Populate ({selectedRequestIds.length})
                              </span>
                            </>
                          )}
                        </Button>
                        ...
                        >
                          {isBulkSubmitting ? (
                            <>
                              <Loader2 className="h-4 w-4 animate-spin" />
                              <span>
                                Submitting... ({bulkProgress.current}/
                                {bulkProgress.total})
                              </span>
                            </>
                          ) : (
                            <>
                              ...
                              <span>
                                Bulk Submit ({selectedRequestIds.length})
                              </span>
                            </>
                          )}
                        </Button>
                      ...
                      <div className="flex items-center space-x-4 text-sm">
                        {bulkOperationResults.successful.length > 0 && (
                          <div className="flex items-center space-x-1 text-green-600">
                            ...
                            <span>
                              {bulkOperationResults.successful.length}{" "}
                              successful
                            </span>
                          </div>
                        )}
                        {bulkOperationResults.failed.length > 0 && (
                          <div className="flex items-center space-x-1 text-red-600">
                            ...
                            <span>
                              {bulkOperationResults.failed.length} failed
                            </span>
                          </div>
                        )}
                      </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                              <div>
                                ...
                                <p className="font-medium">
                                  {request.procedureCode || "N/A"}
                                </p>
                                <p className="text-gray-700">
                                  {request.procedureName ||
                                    "Procedure name not available"}
                                </p>
                              </div>
                                  ...
                                  >
                                    <AccordionTrigger className="px-3 py-2 hover:no-underline bg-blue-50 rounded-t-lg data-[state=open]:rounded-b-none">
                                      <div className="flex items-center justify-between w-full mr-3">
                                        ...
                                        <div className="flex items-center space-x-2 text-xs">
                                          ...
                                          >
                                            {matchingCriteria.timeFrameRequired}{" "}
                                            days processing
                                          </Badge>
                                        </div>
                                      </div>
                                    ...
                                    <AccordionContent className="px-3 pb-3 bg-blue-50 border-t border-blue-200">
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs mt-2">
                                        {matchingCriteria.medicalNecessityCriteria &&
                                          matchingCriteria
                                            .medicalNecessityCriteria.length >
                                            0 && (
                                            <div>
                                        ...
                                        {matchingCriteria.denialReasons &&
                                          matchingCriteria.denialReasons
                                            .length > 0 && (
                                            <div>
                                      ...
                                      <div className="mt-3 pt-2 border-t border-blue-300 flex items-center justify-between text-xs text-blue-700">
                                        <span>
                                          ...
                                          {matchingCriteria.authValidityDays}{" "}
                                          days
                                        </span>
                            ...
                            {!matchingCriteria && (
                              <div className="mt-4 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
                                <div className="flex items-center">
                                  ...
                                  No specific payer criteria found for this
                                  procedure combination. Manual review may be
                                  required.
                                </div>
                              </div>
                            )}
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                console.log(
                                  `Request ${request.id} flagged for review successfully`,
                                );
                                ...
                                console.error(
                                  "Failed to flag request for review:",
                                  error,
                                );
                              }
                            }}
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>
                    Procedure Requirements & Payer Criteria Reference Guide
                  </CardTitle>
                  <p className="text-gray-600 mt-1">
                    Comprehensive reference for authorization requirements,
                    payer policies, and approval criteria
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Search className="h-4 w-4 text-gray-400" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {Array.from(
                        new Set(procedureRequirements.map((p) => p.category)),
                      ).map((category) => (
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={setSelectedPayerFilter}
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-orange-800">
                          {
                            procedureRequirements.filter((p) => p.requiresAuth)
                              .length
                          }
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-green-800">
                          {Math.round(
                            procedureRequirements.reduce(
                              (sum, p) => sum + p.approvalRate,
                              0,
                            ) / procedureRequirements.length,
                          )}
                          %
                    ...
                    const filtered = procedureRequirements.filter((proc) => {
                      const matchesSearch =
                        ...
                        proc.category
                          .toLowerCase()
                          .includes(procedureSearchTerm.toLowerCase()) ||
                        proc.commonDenialReasons.some((reason) =>
                          reason
                            .toLowerCase()
                            .includes(procedureSearchTerm.toLowerCase()),
                        );
                      ...
                      const matchesAuthFilter =
                        ...
                        (selectedPayerFilter === "required" &&
                          proc.requiresAuth) ||
                        (selectedPayerFilter === "not-required" &&
                          !proc.requiresAuth) ||
                        (selectedPayerFilter === "high-risk" &&
                          proc.riskLevel === "High") ||
                        (selectedPayerFilter === "medium-risk" &&
                          proc.riskLevel === "Medium") ||
                        (selectedPayerFilter === "low-risk" &&
                          proc.riskLevel === "Low");

                      return (
                        matchesSearch && matchesCategory && matchesAuthFilter
                      );
                    });

                    return filtered.map((proc) => {
                      // Find matching insurer criteria for this procedure
                      const relatedCriteria = insurerCriteria.filter(
                        (c) => c.procedureCode === proc.procedureCode,
                      );
                          ...
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-xs text-gray-500">
                                {proc.averageProcessingDays} days avg
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-sm">
                            <div className="bg-blue-50 p-3 rounded">
                              ...
                              {proc.requiresAuth && (
                                <p className="text-xs text-blue-600 mt-1">
                                  Submit{" "}
                                  {proc.leadTimeRequired ||
                                    proc.averageProcessingDays}{" "}
                                  days before procedure
                                </p>
                              )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 rounded">
                              ...
                              <p className="text-gray-800">
                                {proc.averageProcessingDays} days average
                              </p>
                              ...
                              <div className="space-y-2">
                                {relatedCriteria.map((criteria, idx) => (
                                  ...
                                  >
                                    ...
                                    {criteria.criteria
                                      .medicalNecessityCriteria &&
                                      criteria.criteria.medicalNecessityCriteria
                                        .length > 0 && (
                                        <div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 mb-4">
                    Performance by Insurance Provider
                  </h3>
      ...
      {selectedRequestForModal && (
        <PreAuthFormModal
          ...
          insurerCriteria={getMatchingCriteria(
            selectedRequestForModal.procedureCode,
            selectedRequestForModal.payer,
          )}
  ...
  function getSamplePatientData(patientId: string) {
    ...
    const samplePatients = {
      "PAT-12345": {
        ...
        clinicalHistory:
          "Patient presents with mechanical symptoms of right knee locking and catching. Conservative treatment with PT for 8 weeks has failed. MRI shows complex medial meniscal tear with functional limitation.",
        priorTreatments:
          "Physical therapy (8 weeks), NSAIDs, activity modification",
      ...
      "PAT-67890": {
        ...
        clinicalHistory:
          "3-month history of exertional chest pain and shortness of breath. Recent stress test positive for ischemia. Family history of CAD.",
        priorTreatments:
          "Medical management with beta-blockers, statins, antiplatelet therapy for 6 months",
        currentMedications:
          "Metoprolol 50mg BID, Atorvastatin 40mg daily, Aspirin 81mg daily",
      ...
      "PAT-11111": {
        ...
        urgency: "routine" as const,
        clinicalHistory:
          "6-month history of lower back pain with left leg radiculopathy. Failed conservative treatment including PT and epidural injections. MRI shows L4-L5 disc herniation with nerve root compression.",
        priorTreatments:
          "Physical therapy (12 weeks), epidural steroid injections (2), NSAIDs",
      ...
      "PAT-77888": {
        ...
        procedureDescription:
          "Injection, anesthetic agent; transforaminal epidural",
        scheduledDate: "2025-10-15",
        urgency: "routine" as const,
        clinicalHistory:
          "C6-C7 radiculopathy with arm pain and weakness. Failed conservative treatment with medications and PT for 8 weeks. MRI shows disc herniation with nerve root compression.",
        priorTreatments:
          "Physical therapy (8 weeks), oral medications, activity modification",
    ...
    return (
      samplePatients[patientId as keyof typeof samplePatients] || {
        patientId: patientId,
        firstName: "Unknown",
        lastName: "Patient",
