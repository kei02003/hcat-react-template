...
import { 
  ...
  DollarSign, FileText, Users, Calendar, Target, Activity,
  HeartHandshake, Shield, Brain, Zap
} from "lucide-react";
...
export function SummaryDashboard() {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'denial': return <Shield className="h-4 w-4 text-red-500" />;
      case 'appeal': return <HeartHandshake className="h-4 w-4 text-blue-500" />;
      case 'filing': return <Calendar className="h-4 w-4 text-orange-500" />;
      case 'collection': return <DollarSign className="h-4 w-4 text-green-500" />;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100">
          <CheckCircle className="w-3 h-3 mr-1" />Completed
        </Badge>;
      case 'pending':
        return <Badge className="bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100">
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Revenue Cycle Summary</h1>
          <p className="text-muted-foreground">
            Comprehensive overview of your healthcare revenue cycle performance
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <CardContent>
            ...
            <div className="flex items-center space-x-1 text-xs">
              ...
              <span className={getTrendColor(mockMetrics.revenueChange)}>
                {mockMetrics.revenueChange > 0 ? '+' : ''}{mockMetrics.revenueChange}%
              </span>
          ...
          <CardContent>
            ...
            <div className="flex items-center space-x-1 text-xs">
              ...
              <span className={getTrendColor(mockMetrics.denialChange, true)}>
                {mockMetrics.denialChange > 0 ? '+' : ''}{mockMetrics.denialChange}%
              </span>
              <span className="text-muted-foreground">vs last period</span>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{mockMetrics.appealSuccessRate}%</div>
            <div className="flex items-center space-x-1 text-xs">
              {getTrendIcon(mockMetrics.appealChange)}
              <span className={getTrendColor(mockMetrics.appealChange)}>
                {mockMetrics.appealChange > 0 ? '+' : ''}{mockMetrics.appealChange}%
              </span>
              <span className="text-muted-foreground">vs last period</span>
            </div>
          </CardContent>
          ...
          <CardContent>
            ...
            <div className="flex items-center space-x-1 text-xs">
              ...
              <span className={getTrendColor(mockMetrics.arChange, true)}>
                {mockMetrics.arChange > 0 ? '+' : ''}{mockMetrics.arChange}%
              </span>
              <span className="text-muted-foreground">vs last period</span>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{mockMetrics.timelyFilingRate}%</div>
            <div className="flex items-center space-x-1 text-xs">
              {getTrendIcon(mockMetrics.timelyFilingChange)}
              <span className={getTrendColor(mockMetrics.timelyFilingChange)}>
                {mockMetrics.timelyFilingChange > 0 ? '+' : ''}{mockMetrics.timelyFilingChange}%
              </span>
              <span className="text-muted-foreground">vs last period</span>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Badge variant="destructive">{mockQuickStats.activeDenials}</Badge>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              ...
              <Badge variant="destructive">{mockQuickStats.overdueFilings}</Badge>
            </div>
          </CardContent>
        </Card>
