    ...
    for (let i = 0; i < data.length; i += BATCH_SIZE) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        try {
          ...
          // Insert into timely_filing_claims table
          await db.execute(sql`
            INSERT INTO timely_filing_claims (
              ...
              current_payor_id,
              current_payor_nm,
              current_financial_class_cd,
              current_financial_class_dsc,
              service_dt,
              billing_dt,
              filing_deadline_dt,
              last_filing_dt,
              ...
              ${currentPayorId},
              ${currentPayorName},
              ${currentFinancialClassCode},
              ${currentFinancialClassDesc},
              ${serviceDate},
              ${billingDate},
              ${filingDeadlineDate},
              ${lastFilingDate},
              ...
              ${new Date()},
              ${new Date()}
            ) ON CONFLICT (timely_filing_id) DO NOTHING
          `);
          success++;
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
